## DROM_BOT - это проект по созданию телеграм-бота, который собирает информацию о пользователе и на ее основе делает запрос на сайт о продаже авто Drom.ru. В качестве ответа бот отправляет пользователю данные из объявлений.
...
#### Структура файлов:
  * **basic_dictionaries** - папка с двумя словарями, в которых содержатся данные о регионах / населенных пунктах и марках / моделях авто. Составлены на основе функций (парсеры) **regions_cities.py** и **brands_models.py**, соответственно
